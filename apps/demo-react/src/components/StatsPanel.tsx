...
function StatsPanel({ state }: StatsPanelProps) {
    ...
    () => [
      {
        ...
        raw: state.currentTime.toFixed(3),
      },
      { label: 'Duration', value: formatTime(state.duration), raw: state.duration.toFixed(3) },
      {
        label: 'Paused',
        value: state.paused ? 'Yes' : 'No',
        status: state.paused ? 'warning' : 'success',
      },
      {
        label: 'Ended',
        value: state.ended ? 'Yes' : 'No',
        status: state.ended ? 'warning' : undefined,
      },
      {
        label: 'Seeking',
        value: state.seeking ? 'Yes' : 'No',
        status: state.seeking ? 'warning' : undefined,
      },
  ...
  const volumeStats = useMemo<StatItem[]>(
    () => [
      ...
      {
        label: 'Muted',
        value: state.muted ? 'Yes' : 'No',
        status: state.muted ? 'warning' : undefined,
      },
    ],
  ...
  const networkStats = useMemo<StatItem[]>(
    () => [
      ...
      {
        label: 'Current Src',
        value: state.currentSrc ? truncateUrl(state.currentSrc) : 'none',
        title: state.currentSrc,
      },
    ],
    [
      state.networkStateText,
      state.networkState,
      state.readyStateText,
      state.readyState,
      state.currentSrc,
    ]
  );
  ...
  const capabilityStats = useMemo<StatItem[]>(
    () => [
      {
        label: 'Can Play MP4',
        value: state.canPlayType_mp4 || 'no',
        status: getCanPlayStatus(state.canPlayType_mp4),
      },
      {
        label: 'Can Play WebM',
        value: state.canPlayType_webm || 'no',
        status: getCanPlayStatus(state.canPlayType_webm),
      },
      {
        label: 'Can Play HLS',
        value: state.canPlayType_hls || 'no',
        status: getCanPlayStatus(state.canPlayType_hls),
      },
    ],
  ...
  const codecStats = useMemo<StatItem[]>(
    () => [
      {
        label: 'H.264 (AVC)',
        value: state.codec_h264 ? 'Yes' : 'No',
        status: state.codec_h264 ? 'success' : 'error',
      },
      {
        label: 'H.265 (HEVC)',
        value: state.codec_h265 ? 'Yes' : 'No',
        status: state.codec_h265 ? 'success' : 'warning',
      },
      {
        label: 'VP9',
        value: state.codec_vp9 ? 'Yes' : 'No',
        status: state.codec_vp9 ? 'success' : 'warning',
      },
      {
        label: 'AV1',
        value: state.codec_av1 ? 'Yes' : 'No',
        status: state.codec_av1 ? 'success' : 'warning',
      },
    ],
